...
public partial class Pages_Set_Category : System.Web.UI.Page
{
    ...
    {
        DataTable dt = categoryManager.GetCategory();
    ...
    private void AddCategory(string categoryName, string categoryDescription)
    {
        if (ddlCategory.SelectedValue == "0")
        {
            Page.ClientScript.RegisterStartupScript
                (GetType(), "msgbox", "alert('" + "Select A Caregory!" + "');", true);
            return;
        }
        ...
        insert[8] = ddlCategory.SelectedValue;

        bool save = categoryManager.InsertSubCategoryBook(insert);
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            AddCategory(txtCategoryName.Text, txtDescription.Text);
            Response.Redirect(Request.RawUrl);
        }
    ...
    {
        DataTable dt = categoryManager.GetAllSubCategory();
    ...
    {
        var result = categoryManager.getSubCategoryBook(categoryId);
    ...
    private void GetCategoryByIdForDetailView(int categoryId)
    {
        ...
        {
            //var category = result.First();
            lblCategoryView.Text= result.Rows[0]["Category"].ToString();
    ...
    protected void gridViewCategory_RowCommand(object sender, GridViewCommandEventArgs e)
    {
            ...
            && e.CommandName != "DetailCategory" && e.CommandName != "ActivateDeactivate")
        {
            ...
            categoryManager.DeleteSubCategory(categoryId);
            GetAllCategoryRecord();
        }
            ...
            && e.CommandName != "EditCategory" && e.CommandName != "DetailCategory")
        {
            ...
            if (result.Rows.Count > 0)
            {
                ...
                var data = categoryManager.getSubCategoryBook(categoryId);
                var timeZone = TimeZoneInfo.FindSystemTimeZoneById("Bangladesh Standard Time");
                var now = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, timeZone);
                string date = now.ToString("yyyy-MM-dd hh:mm:ss tt");
                ...
                if (data.Rows[0]["Status"].ToString() == "Active")
                {
                    ...
                    insert[1] = "Admin";
                    insert[2] = date;
                    bool update = categoryManager.UpdateSubCategoryStatus(insert, categoryId);
                    GetAllCategoryRecord();
                }
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        insert[3] = date;
        insert[8] = ddlCategory.SelectedValue;
        categoryManager.UpdateSubCategory(insert, categoryId);
        Response.Redirect(Request.RawUrl);
    ...
    {
        DataTable dt = categoryManager.SearchSubCategoryBookByName(txtSearch.Text);
        //gridViewCategory.DataSourceID = "SqlDataSource1";
        gridViewCategory.DataSource = dt;
        gridViewCategory.DataBind();
    }
