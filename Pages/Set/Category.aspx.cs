...
public partial class Pages_Set_Category : System.Web.UI.Page
{
    ...
    private void AddCategory(string categoryName, string categoryDescription)
    {
        ...
        bool save = categoryManager.InsertCategoryBook(insert);
    }
    ...
    {
        DataTable dt = categoryManager.GetAllCategory();
    ...
    {
        var result = categoryManager.getCategoryBook(categoryId);
    ...
    protected void gridViewCategory_RowCommand(object sender, GridViewCommandEventArgs e)
    {
            ...
            && e.CommandName != "EditCategory" && e.CommandName != "DetailCategory")
        {
            ...
            if (result.Rows.Count > 0)
            {
                ...
                var data = categoryManager.getCategoryBook(categoryId);
                var timeZone = TimeZoneInfo.FindSystemTimeZoneById("Bangladesh Standard Time");
                var now = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, timeZone);
                string date = now.ToString("yyyy-MM-dd hh:mm:ss tt");
                ...
                if (data.Rows[0]["Status"].ToString() == "Active")
                {
                    ...
                    insert[1] = "Admin";
                    insert[2] = date;
                    bool update = categoryManager.UpdateCategoryStatus(insert, categoryId);
                    GetAllCategoryRecord();
                }
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        insert[2] = "Admin";
        insert[3] = date;
        categoryManager.UpdateCategory(insert, categoryId);
        Response.Redirect(Request.RawUrl);
    ...
    {
        var result = categoryManager.getCategoryBookByName(txtSearch.Text);
