...
public partial class Pages_Books : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString.ToString().Contains("SubCat") || Request.QueryString.ToString().Contains("Cat"))
            {
    ...
    {
        DataTable dt = bookManager.GetallActiveBooks();
        repeaterallBooks.DataSource = dt;
        repeaterallBooks.DataBind();
    }
    ...
    private void GetBooks()
    {
        ...
        DataTable dt = null;
        if (Request.QueryString.ToString().Contains("SubCat"))
        {
            string id = Request.QueryString["SubCat"].ToString();
            dt = bookManager.GetAllBooksBySubCat(id);
        }
        else if (Request.QueryString.ToString().Contains("Cat"))
        {
            string id = Request.QueryString["Cat"].ToString();
            dt = bookManager.GetAllBooksByCategory(id);
        }
        if (dt.Rows.Count > 0)
        {
    ...
    {
        string publisher = Request.QueryString["Pub"].ToString();
        DataTable dt = null;
        ...
        {
            dt = bookManager.GetAllBooksLowToHigh(publisher);
        }
        else if (ddlSort.SelectedValue == "6")
        {
            dt = bookManager.GetAllBooksHighToLow(publisher);
        }
        if (dt.Rows.Count > 0)
        {
            //Create the PagedDataSource that will be used in paging
    ...
    protected void btnFilter_Click(object sender, EventArgs e)
    {
        ...
        {
            string id = Request.QueryString["Pub"].ToString();
            DataTable dt = bookManager.GetPublicationAllBookByPriceRange(id, min, max);
            if (dt.Rows.Count > 0)
            {
                //Create the PagedDataSource that will be used in paging
                PagedDataSource pgitems = new PagedDataSource();
